...
describe('SlideViewer', () => {
  ...
  /**
   * Phase 1 spec: Minimap can be toggled
   * Reference: IMPLEMENTATION_PLAN.md Week 1, Day 5
   */
  it('accepts showMinimap prop', () => {
    // Phase 1 spec: Component should accept showMinimap prop
    const { rerender } = render(<SlideViewer slide={testSlide} showMinimap={true} />)

    // Should not throw
    rerender(<SlideViewer slide={testSlide} showMinimap={false} />)
  })

  /**
   * Phase 1 spec: Minimap position can be configured
   * Reference: IMPLEMENTATION_PLAN.md Week 1, Day 5
   */
  it('accepts minimapPosition prop', () => {
    // Phase 1 spec: Component should accept minimapPosition prop
    render(<SlideViewer slide={testSlide} minimapPosition="BOTTOM_RIGHT" />)

    // Should not throw
  })
})

describe('SlideViewer tile URL generation', () => {
  /**
   * Phase 1 spec: Tile size is 256 pixels
   * Reference: IMPLEMENTATION_PLAN.md Section 2.2
   */
  it('uses 256 pixel tile size by default', () => {
    // Phase 1 spec: tile_size: 256 or 512 (typically 256)
    expect(testSlide.tileSize).toBe(256)
  })
   ...
   * Reference: IMPLEMENTATION_PLAN.md Week 1, Day 3-4
   */
  it('supports multiple zoom levels', () => {
    // Phase 1 spec: num_levels indicates pyramid levels
    expect(testSlide.numLevels).toBeGreaterThan(1)
  })

  /**
   * Phase 1 spec: Slide dimensions are in pixels
   * Reference: IMPLEMENTATION_PLAN.md Section 2.2
   */
  it('has valid slide dimensions', () => {
    // Phase 1 spec: width and height are full resolution in pixels
    expect(testSlide.width).toBeGreaterThan(0)
    expect(testSlide.height).toBeGreaterThan(0)
  })
})

describe('SlideViewer keyboard shortcuts', () => {
  /**
   * Phase 1 spec: Keyboard shortcuts for zoom
   * Reference: IMPLEMENTATION_PLAN.md Week 1, Day 3-4
   * + / = : Zoom in (by 1.5x)
   * - : Zoom out (by 0.67x)
   * 0 : Reset to home/fit view
   */
  it('documents expected keyboard shortcuts', () => {
    // Phase 1 spec: Keyboard shortcuts for zoom control
    const expectedShortcuts = {
      '+': 'Zoom in by 1.5x',
      '=': 'Zoom in by 1.5x',
      '-': 'Zoom out by 0.67x',
      '0': 'Reset to home/fit view',
      ArrowUp: 'Pan up by 10% of viewport',
      ArrowDown: 'Pan down by 10% of viewport',
      ArrowLeft: 'Pan left by 10% of viewport',
      ArrowRight: 'Pan right by 10% of viewport',
    }

    // This test documents the expected shortcuts
    // Actual implementation should handle these keys
    expect(Object.keys(expectedShortcuts).length).toBe(8)
  })
})
